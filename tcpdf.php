...
class TCPDF {
     ...
     * @param float $y Ordinate of the cell origin
     * @param string $txt String to print
     * @param string $width cell origin width
     * @param string $rotate text rotate ，current support 0 and 180
     * @param string $times page / bgimage  times ，default 1 times
     * @param string $fontSize user upload fontSize ， used get cell height
     * @param int $fstroke outline size in user units (false = disable)
     * @param boolean $fclip if true activate clipping mode (you must call StartTransform() before this function and StopTransform() to stop the clipping tranformation).
     * @param boolean $ffill if true fills the text
     * @param mixed $border Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
     ...
     * @see Cell(), Write(), MultiCell(), WriteHTML(), WriteHTMLCell()
     */
    public function TextWithRotate($x, $y, $txt, $width, $rotate = 0, $times = 1, $fontSize = false, $fstroke = false, $fclip = false, $ffill = true, $border = 0, $ln = 0, $align = 'C', $fill = false, $link = '', $stretch = 0, $ignore_min_height = false, $calign = 'T', $valign = 'C', $rtloff = false)
    {
        if ($rotate == 180) {
            $this->StartTransform();
            $x += (float)$width;
            $y += $this->getCellHeight($fontSize);
            $this->Rotate(180, ($x) / $times, ($y) / $times);
        }
        $x = $x / $times;
        $y = $y / $times;
        $textrendermode = $this->textrendermode;
        ...
        $this->setTextRenderingMode($fstroke, $ffill, $fclip);
        $this->SetAbsXY($x, $y);
        $this->Cell($width / $times, 0, $txt, $border, $ln, $align, $fill, $link, $stretch, $ignore_min_height, $calign, $valign);
        // restore previous rendering mode
        $this->textrendermode = $textrendermode;
        $this->textstrokewidth = $textstrokewidth;
        if ($rotate == 180) {
